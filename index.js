...
app.get('/api/gpt4', async (req, res) => {
  try {
    ...
    const response = await ptz.chat(message);
    res.status(200).json({
      status: 200,
      creator: "KyuuRzy",
      data: { response }
...
// Endpoint untuk degreeGuru
app.get('/api/itp', async (req, res) => {
  try {
    const { message }= req.query;
    if (!message) {
      return res.status(400).json({ error: 'Parameter "message" tidak ditemukan' });
    }
    const response = await ptz.imgToPrompt(message);
    res.status(200).json({
      status: 200,
      creator: "KyuuRzy",
      data: { response }
...
app.get('/api/tiktoks', async (req, res) => {
  try {
    ...
    const response = await ptz.tiktoks(message);
    res.status(200).json({
      status: 200,
      creator: "KyuuRzy",
      data: { response }
